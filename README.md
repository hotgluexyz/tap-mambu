...
| `transactions` | **Child of `journal_entries`.** For each parent row with a `transactionId`, calls `GET /loans/transactions/{id}` when `productType` is `LOAN`, or `GET /deposits/transactions/{id}` when `productType` is `SAVINGS`, with `detailsLevel=FULL`. Rows without a `transactionId` skip the child request. Any other `productType` with a transaction id raises an error. Within a single sync, the same `(loans|deposits, transaction id)` is fetched at most once; extra journal lines reuse the cached response with their own `journal_entry_encoded_key`. | `journal_entry_encoded_key`, `encodedKey` | — |
